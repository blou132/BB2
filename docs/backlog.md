...
| US2 | En tant qu’administrateur, je veux gérer les équipes afin d’associer joueurs et Grand Prix. | Haute | CRUD complet, sélection d’un Grand Prix existant, upload de logo |
| US3 | En tant qu’administrateur, je veux gérer les joueurs pour préparer les feuilles de course. | Haute | CRUD complet, sélection d’une équipe, upload de portrait |
| US4 | En tant que spectateur, je veux visualiser les joueurs par équipe. | Moyenne | Vue read-only `?route=jointure`, tri par équipe |
